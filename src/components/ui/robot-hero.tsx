...
function RobotEye({ position, rotation, scale = 1, blinkDuration = 0.15, blinkCycle = 3.0, isLovedRef }: { position: [number, number, number], rotation: [number, number, number], scale?: number, blinkDuration?: number, blinkCycle?: number, isLovedRef: React.RefObject<boolean> }) {
...
function RobotPrototype({ neckParams = { baseR: 0.25, baseH: -0.01, midR: 0.23, midH: 0.02, lipBottomR: 0.27, lipBottomH: 0.025, lipTopR: 0.28, lipTopH: 0.05, innerR: 0.24, innerDropH: 0.03 }, bodyParams = { bodyBevelR: 0.21, bodyBevelY: 0.38, bodyBevelT: 0.015 } }: { neckParams?: Record<string, number>, bodyParams?: Record<string, number> }) {
  ...
  useFrame((state, delta) => {
    ...
    // Static cursor position (no mouse tracking in hero)
    const tx = 0;
    const ty = 0;
...
export function RobotHero({
  backgroundText = 'KRISHNAM',
  navItemsLeft = [
    { label: 'About', href: '#about-section' },
    { label: 'Skills', href: '#skills-section' },
    { label: 'Projects', href: '#projects-section' },
    { label: 'Arcade', href: '#arcade-section' },
  ],
  contactText = 'Contact',
  contactHref = '#contact-section',
  contactTarget,
  ...
  const entorno = {
    ...
    luzAmbiente: 0.75,
    luzPrincipal: 0.0,
    luzPrincipalColor: '#ff7b00',
    luzRelleno: 0.0,
    luzRellenoColor: '#ffaa00',
    sombraOpacidad: 0.85,
    sombraBlur: 1.7
  };
